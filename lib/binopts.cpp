...
extern "C" {
    ...
    unsigned int rpiLop_div(unsigned int dividend, unsigned int divisor)
    {
        ...
        for(t=quotient;bits_left;t=quotient,bits_left--)
        {
            ...
            (remainder >= divisor)?remainder = rpiLop_sub(remainder,divisor),quotient= rpiLop_add(quotient,1):quotient=quotient;//conditional subtract and add
        }
    ...
    unsigned long long int rpiLop_div64(unsigned long long int dividend, unsigned long long int divisor)
    {
        unsigned long long int quotient,remainder,t;
        long long int bits_left = 8 * sizeof(unsigned long long int);
        quotient = dividend;
        remainder = 0;
        //for(;bits_left;t=quotient,quotient = rpiLop_add(quotient,quotient); remainder = rpiLop_add(rpiLop_add(remainder,remainder),(quotient<t)),((remainder >= divisor)?remainder = rpiLop_subtract(rem,divisor),quotient= rpiLop_add(quotient,1):quotient=quotient),bits_left--);
        for(t=quotient;bits_left;t=quotient,bits_left--)
